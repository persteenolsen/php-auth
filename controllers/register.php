    ...
    if(isset($_POST["submit"])) {
        ...
        if(!empty($firstname) && !empty($lastname) && !empty($email) && !empty($mobilenumber) && !empty($password)){
            ...
            if($rowCount > 0) {
                ...
                // perform validation
                if(!preg_match("/^[a-zA-Z ]{2,10}$/", $_first_name)) {
                    $f_NameErr = '<div class="alert alert-danger">
                            Only 2-10 letters including white space allowed.
                        </div>';
                }
                if(!preg_match("/^[a-zA-Z ]{2,10}$/", $_last_name)) {
                ...
                if((preg_match("/^[a-zA-Z ]{2,10}$/", $_first_name)) && (preg_match("/^[a-zA-Z ]{2,10}$/", $_last_name)) &&
                 ...
                 (preg_match("/^(?=.*\d)(?=.*[@#\-_$%^&+=§!\?])(?=.*[a-z])(?=.*[A-Z])[0-9A-Za-z@#\-_$%^&+=§!\?]{8,20}$/", $_password))){
                    ...
                    if($sqlQuery) {
                        $msg = 'Click on the activation link to verify your email. <br><br>
                          <a href="https://phpauth.persteenolsen.dk/user_verificaiton.php?token='.$token.'"> Click here to verify email</a>
                        ';

                        // Create the Transport
                       
                        $transport = (new Swift_SmtpTransport('yoursmtphost', 587))
                        ->setUsername('youremail-username')
                        ->setPassword('yourpassword');
                        $mailer = new Swift_Mailer($transport);
                        $message = (new Swift_Message('Please Verify Email Address!'))
                        ->setFrom(['youremailfrom' ]) 
